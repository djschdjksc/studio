  ...
  const handleLocalKeyDown = (e: React.KeyboardEvent<HTMLInputElement>) => {
    if (showSuggestions) {
      if (e.key === 'ArrowDown') {
        ...
        } else if (suggestions.length > 0 && activeIndex === -1) {
          // If user presses enter without navigating, select the first suggestion
          e.preventDefault();
  ...
  return (
    <div className="relative w-full">
      <Input
        ...
        onFocus={() => setInputValue('')} // Clear on focus to allow re-searching
        onBlur={() => {
            // Give time for select to register before hiding
            setTimeout(() => {
                setShowSuggestions(false);
                if (!items.some(i => i.name.toLowerCase() === inputValue.toLowerCase())) {
                  setInputValue(value); // Reset if not a valid selection
                }
            }, 150);
        }}
